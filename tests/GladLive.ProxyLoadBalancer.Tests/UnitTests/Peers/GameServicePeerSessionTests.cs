...
namespace GladLive.ProxyLoadBalancer.Tests
{
	...
	public static class GameServicePeerSessionTests
	{
		...
		public static void Test_Ctor_Doesnt_Throw_On_Non_Null_Parameters()
		{
			...
			//assert
			Assert.DoesNotThrow(() => new GameServicePeerSession(Mock.Of<ILog>(), Mock.Of<INetworkMessageRouterService>(),
				Mock.Of<IConnectionDetails>(), Mock.Of<INetworkMessageSubscriptionService>(), Mock.Of<IDisconnectionServiceHandler>(), Mock.Of<INetworkMessageRouteBackService>(), Mock.Of<IRequestMessageHandlerService<GameServicePeerSession>>()));
		}
		...
		public static void Test_Ctor_Throws_On_Null_Handler_Service()
		{
			...
			//I checked the wrong object for null.
			//assert
			Assert.Throws<ArgumentNullException>(() => new GameServicePeerSession(Mock.Of<ILog>(), Mock.Of<INetworkMessageRouterService>(),
		...
		{
			//arrange
			Mock<IRequestMessageHandlerService<GameServicePeerSession>> handler = new Mock<IRequestMessageHandlerService<GameServicePeerSession>>(MockBehavior.Loose);
			GameServicePeerSession session = new GameServicePeerSession(Mock.Of<ILog>(), Mock.Of<INetworkMessageRouterService>(),
		...
		public static void Test_Session_Throws_Null_Arg_OnRequestRecieve_When_Payload_Is_Null()
		{
			//arrange
			GameServicePeerSession session = new GameServicePeerSession(Mock.Of<ILog>(), Mock.Of<INetworkMessageRouterService>(),
				new Mock<IConnectionDetails>(MockBehavior.Loose).Object, Mock.Of<INetworkMessageSubscriptionService>(), Mock.Of<IDisconnectionServiceHandler>(), Mock.Of<INetworkMessageRouteBackService>(), Mock.Of<IRequestMessageHandlerService<GameServicePeerSession>>());
		...
		public static Action<IRequestMessage, IMessageParameters> GrabProtectedOnRecieveRequestMethod(GameServicePeerSession session)
		{
