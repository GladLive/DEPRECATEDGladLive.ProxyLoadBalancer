...
namespace GladLive.ProxyLoadBalancer
{
	...
	public static class AutoFacTests
	{
		...
		public static void Test_GameSessionHandlerModule_Registers_Contravariant_Handlers()
		{
			...
			Assert.Contains(typeof(PayloadDebugLoggingHandler), (ICollection)container.ResolveNamed<IEnumerable<IRequestMessageHandler<GameServicePeerSession>>>("default").Select(x => x.GetType()).ToArray());
		...
		public static void Test_ClientSessionHandlerModule_Registers_Contravariant_Handlers()
		{
			...
			//assert
			Assert.Contains(typeof(PayloadDebugLoggingHandler), (ICollection)container.Resolve<IEnumerable<IRequestMessageHandler<UserClientPeerSession>>>().Select(x => x.GetType()).ToArray());
